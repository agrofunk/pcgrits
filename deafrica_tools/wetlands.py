...
def WIT_drill(
    ...
    with HiddenPrints():  # suppres the prints from this func
        tcw = calculate_indices(ds_ls, index=["TCW"], normalise=False, satellite_mission="ls", drop=True)
    ...
    NoData_count = ((100 - wofs_area_percent - tcw_less_wofs - PV_percent - NPV_percent - BS_percent) / 100) * pixels
        ...
        if ((len(bands) == 3) | (len(bands) == 1)) & all([(b in ds.data_vars) for b in bands]):
            ...
            df1 = pd.DataFrame(
                {
                    ...
                    "green_veg_percent": df.wofs_area_percent + df.wet_percent + df.green_veg_percent,
            ...
            def update_figure(frame_i):
                ...
                for i, line in enumerate(line_test.lines):
                    ...
                    y = df1[df1.index <= datetime(year=year, month=month, day=day, hour=23, minute=59)].iloc[:, i]
                    x = df1[df1.index <= datetime(year=year, month=month, day=day, hour=23, minute=59)].index
...
def _ds_to_arrraylist(ds, bands, time_dim, x_dim, y_dim, percentile_stretch, image_proc_func=None):
    ...
    for i, timestep in enumerate(ds[time_dim]):
        ...
        if len(bands) == 1:
            ...
            img_toshow = exposure.rescale_intensity(ds_i[bands[0]].values, in_range=(p_low, p_high), out_range="image")
            ...
            img_toshow = exposure.rescale_intensity(rawimg, in_range=(p_low, p_high), out_range=(0, 1.0))
