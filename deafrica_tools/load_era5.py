...
def load_era5(
    ...
    while month <= date_to.astype("datetime64[M]"):
        ...
        ds = xr.open_zarr(
            fsspec.get_mapper(url, anon=True, client_kwargs={"region_name": "us-east-1"}), consolidated=True
        )
        ...
        if "time1" in ds.dims:
            ds = ds.rename({"time1": "time"})  # This should INTENTIONALLY error if both times are defined
