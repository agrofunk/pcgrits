...
class transect_app(HBox):
    def __init__(self):
        ...
        # Create map widget
        self.m = deawidgets.create_map(map_center=(0.5273, 25.1367), zoom_level=3, basemap=basemaps.Esri.WorldImagery)
    ...
    def update_fileupload_transects(self, change):
        ...
        with self.status_info:
            try:
                ...
                transect_gdf = gpd.read_file(valid_file).to_crs("EPSG:4326").explode().reset_index(drop=True)
            ...
            if transect_gdf is not None:
                ...
                if deacl_gdf is not None:
                    ...
                    dist_df = transect_distances(
                        transect_gdf.to_crs("EPSG:6933"), deacl_gdf.to_crs("EPSG:6933"), mode=self.mode
                    )
