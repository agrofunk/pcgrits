...
def export_image_app(
    ...
    sat_params = {
        "Landsat-9": {
            "products": ["ls9_sr"],
            "resolution": [-30, 30],
            "styles": {"True colour": ["red", "green", "blue"], "False colour": ["swir_1", "nir", "green"]},
        },
        "Landsat-8": {
            "products": ["ls8_sr"],
            "resolution": [-30, 30],
        ...
        "Sentinel-2": {
            "products": ["s2_l2a"],
            "resolution": [-10, 10],
            "styles": {"True colour": ["red", "green", "blue"], "False colour": ["swir_2", "nir_1", "green"]},
        },
    ...
    load_params = {"output_crs": crs, "resolution": sat_params[satellites]["resolution"], "resampling": "bilinear"}
    ...
    fname = f"{sensor} - {date} - {site} - {style}, " f"{load_params['resolution'][1]} m resolution.{output_format}"
    ...
    rgb_rescaled = exposure.rescale_intensity(rgb_array.astype(float), in_range=(vmin, vmax), out_range=(0.0, 1.0))
