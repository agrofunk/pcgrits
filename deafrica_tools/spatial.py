...
def xr_vectorize(
    ...
    **rasterio_kwargs :
    ...
    if type(da) is np.ndarray:
        vectors = rasterio.features.shapes(source=da.astype(dtype), transform=transform, **rasterio_kwargs)

    else:
        # Run the vectorizing function
        vectors = rasterio.features.shapes(source=da.data.astype(dtype), transform=transform, **rasterio_kwargs)
    ...
    gdf = gpd.GeoDataFrame(data={attribute_col: values}, geometry=polygons, crs=str(crs))
...
def xr_rasterize(
    ...
    **rasterio_kwargs :
    ...
    arr = rasterio.features.rasterize(shapes=shapes, out_shape=(y, x), transform=transform, **rasterio_kwargs)
...
def subpixel_contours(
    ...
    if len(da.shape) == 2:
        ...
        dim = "z_value"
        contour_arrays = {str(i)[0:10]: contours_to_multiline(da, i, min_vertices) for i in z_values}
        ...
        if len(z_values) > 1:
            raise ValueError(
                "Please provide a single z-value when operating " "in single z-value, multiple arrays mode"
            )

        contour_arrays = {
            str(i)[0:10]: contours_to_multiline(da_i, z_values[0], min_vertices) for i, da_i in da.groupby(dim)
        }
    ...
    contours_gdf = gpd.GeoDataFrame(data=attribute_df, geometry=list(contour_arrays.values()), crs=crs)
...
def reverse_geocode(coords, site_classes=None, state_classes=None):
    ...
    try:
        ...
        default_site_classes = ["city", "town", "village", "suburb", "hamlet", "county", "municipality"]
