...
def the_func(path: str, filename: str, layers: str | None = None) -> None:
    print(path, filename, layers)
    return


the_func(path=1, filename=2, layers=3)


def the_func(path: str, filename: str, layers: str) -> None:
    print("""
    ...
    for var in list(ds.data_vars):
        print(var, ds[var].quantile([0.01, 0.1, 0.5, 0.9, 0.99], skipna=True).values, "\n")
    ...
    if smooth:
        print("smoothening...")
        ds = ds.rolling(time=w, center=True).mean(savgol_filter, window=w, polyorder=2)
...
if __name__ == "__main__":
    path = sys.argv[1]
    return a_funcao_do_denis(path, filename, param, param2, param3)
